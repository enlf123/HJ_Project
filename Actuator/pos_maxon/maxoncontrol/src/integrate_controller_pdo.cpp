...
if (cnt <5/hz) dxl_goal_position = 0.00;
	...
	dxl_goal_position = (M_PI)*cos(2*M_PI*0.05*(cnt-5/hz)*hz)-M_PI;
...
//    setsockopt(s, SOL_CAN_RAW, CAN_RAW_FILTER, NULL, 0);
	...
	for (int j=0; j<100;j++) {
		for (int i=0; i<Id_length;i++) {
			frame.can_id = 0x481 | CAN_RTR_FLAG;
				...
				msg.realPos = float(float(hexarray_to_int4_2(frame_get.data))/mot_pulse_ratio);
    ...
    while (ros::ok())
    {
        ...
        if ((begin-pre).toSec()>hz){
		    ...
		    for (int i = 0; i < Id_length; i++) {
			    ...
			    // Convert dec to hex
				d = int(msg.command*mot_pulse_ratio);
			    val_desired_hex[i] = dec_to_hex(d, 4);
				//printf("%d\n",d);
			    // Send position control command to EPOS
			    frame.can_id = COB_ID_Rx[i][2];
			    frame.can_dlc = 6;
	...
	//            		required_mtu = parse_canframe((char*)(rtr.c_str()), &frame_fd);
				...
				ros::Duration(0.00005).sleep();
				nnbytes = recvmsg(s, &canmsg, 0);
				recv = hexarray_to_int4_2(frame_get.data);
				//ROS_INFO("Receive : %d", recv);
				if ((size_t) nnbytes != CAN_MTU && (size_t) nnbytes != CANFD_MTU) {}
				else {
		            		//msg.realPos = pulsetorad(hexarray_to_int4_2(frame_get.data))-mag_offset;
				    //msg.realPos = float(float(recv)/mot_pulse_ratio);
				    msg.realPos = float(float(hexarray_to_int4_2(frame_get.data))/mot_pulse_ratio);
					msg.motPos = float(float(hexarray_to_int4_3(frame_get.data))/mot_pulse_ratio);
				}
			}
